...
template <size_t N>
void ProgramData_displayIntegrals(std::unique_ptr<ProgramData<N>> & data_ptr) noexcept {
    std::printf("\n\nCoeff XX\n");
    for(size_t idx = 0; idx < 3; ++idx){
        for(size_t jdx = 0; jdx < 3; ++jdx){
            std::printf("\t%f", data_ptr->integrals.coeff_xx[idx][jdx]);
        }
        std::printf("\n");
    }
    ...
    for(size_t idx = 0; idx < 3; ++idx){
        for(size_t jdx = 0; jdx < 3; ++jdx){
            std::printf("\t%f", data_ptr->integrals.coeff_xy[idx][jdx]);
        }
        std::printf("\n");
    }
    ...
    for(size_t idx = 0; idx < 3; ++idx){
        for(size_t jdx = 0; jdx < 3; ++jdx){
            std::printf("\t%f", data_ptr->integrals.coeff_yy[idx][jdx]);
        }
        std::printf("\n");
    }
}
